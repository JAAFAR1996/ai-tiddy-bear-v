    ...
    class MockConfig:
        def __init__(self):
            self.DATABASE_URL = os.getenv(
                "DATABASE_URL",
                "postgresql+asyncpg://username:password@localhost/ai_teddy_bear",
            )
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(
            f'database_cleanup_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
        ),
    ],
)
...
class DatabaseCleanupValidator:
    ...
    async def validate_schema_integrity(self) -> List[str]:
        ...
        try:
            ...
            test_tables = [
                ...
                for table in tables
                if any(
                    pattern in table.lower()
                    for pattern in ["test", "demo", "sample", "dev", "staging"]
                )
            ]
            ...
            async with self.session_factory() as session:
                ...
                if table in ["children"]:
                    name_conditions = [
                        ...
                        for pattern in self.TEST_PATTERNS["names"]
                    ]
                ...
                for pattern in self.TEST_PATTERNS["metadata_patterns"]:
                    metadata_conditions.append(
                        f"metadata_json::text ILIKE '%{pattern}%'"
                    )
    ...
    async def clean_test_data_from_table(
        self, table: str, test_record_ids: List[str]
    ) -> CleanupResult:
            ...
            async with self.session_factory() as session:
                ...
                if table == "children":
                    ...
                    for issue in coppa_issues:
                        coppa_violations.append(
                            f"Child {issue[1]} (ID: {issue[0]}) lacks proper consent/verification"
                        )
        ...
        try:
            async with self.session_factory() as session:
                ...
                for violation in consent_violations:
                    violations.append(
                        f"Child {violation[1]} (age {violation[2]}) lacks parental consent but has logging enabled"
                    )

                # Check for children with invalid ages (outside COPPA range)
                age_check = await session.execute(
                    text(
                        """
                    SELECT c.id, c.name, c.estimated_age 
                    FROM children c 
                ...
                for violation in age_violations:
                    violations.append(
                        f"Child {violation[1]} has invalid age: {violation[2]} (must be 3-13)"
                    )

                # Check for data retention violations
                retention_check = await session.execute(
                    text(
                        """
                ...
                for violation in retention_violations:
                    violations.append(
                        f"Child {violation[1]} has excessive data retention: {violation[2]} days"
                    )

                # Check for missing consent records
                missing_consent_check = await session.execute(
                    text(
                        """
                    SELECT c.id, c.name 
                    FROM children c 
                ...
                for violation in missing_consent:
                    violations.append(
                        f"Child {violation[1]} marked as consented but no consent record found"
                    )

                logger.info(
                    f"✅ COPPA compliance check completed. Violations: {len(violations)}"
                )
                return violations
    ...
    async def run_comprehensive_cleanup(self) -> ValidationResult:
        ...
        try:
            ...
            for table in tables_to_check:
                test_data_identification[table] = (
                    await self.identify_test_data_in_table(table)
                )
            ...
            for table in cleanup_order:
                if table in test_data_identification:
                    ...
                    cleanup_result = await self.clean_test_data_from_table(
                        table, test_ids
                    )
            ...
            total_test_found = sum(
                result.test_records_found for result in cleanup_results
            )
            total_test_deleted = sum(
                result.test_records_deleted for result in cleanup_results
            )
            ...
            is_production_ready = (
                len(all_issues) == 0
                and len(all_coppa_issues) == 0
                and total_test_found == total_test_deleted
            )
    ...
    async def generate_cleanup_report(self, result: ValidationResult):
        ...
        # Save JSON report
        report_file = (
            f"database_cleanup_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        ...
        print(
            f"🎯 Production Ready: {'✅ YES' if result.is_production_ready else '❌ NO'}"
        )
        print()
        ...
        if result.cleanup_results:
            ...
            for cr in result.cleanup_results:
                print(f"   {cr.table}:")
                print(
                    f"     - Test records: {cr.test_records_found} found, {cr.test_records_deleted} deleted"
                )
                print(
                    f"     - Production records remaining: {cr.production_records_remaining}"
                )
                if cr.issues_found:
