...
async def validate_parent_authorization(db: AsyncSession, parent_id: str, user_role: str = 'parent') -> User:
    ...
    if not parent:
        raise AuthorizationError(f"Parent account not found, inactive, or insufficient permissions")
    
    return parent


async def validate_child_access(db: AsyncSession, child_id: str, parent_id: str) -> Child:
    """Validate child exists and parent has access."""
    try:
        child_uuid = uuid.UUID(child_id)
        parent_uuid = uuid.UUID(parent_id)
    except (ValueError, TypeError):
        raise ValidationError(f"Invalid ID format: child_id={child_id}, parent_id={parent_id}")
...
async def validate_child_ownership(db: AsyncSession, child_id: str, parent_id: str) -> Child:
...
def calculate_child_safety_score(child: Child, recent_interactions=None, safety_reports=None) -> float:
    """Calculate comprehensive safety score for child."""
    base_score = 100.0
    
    # Penalty for unresolved safety reports
    unresolved_reports = safety_reports or []
    high_severity_count = sum(1 for r in unresolved_reports if r.severity in ['high', 'critical'] and not r.resolved)
    medium_severity_count = sum(1 for r in unresolved_reports if r.severity == 'medium' and not r.resolved)
    ...
    if not unresolved_reports and (not recent_interactions or not any(i.flagged for i in recent_interactions)):
...
class ChildCreateRequest(BaseModel):
    ...
    estimated_age: int = Field(..., ge=3, le=18, description="Child's estimated age (3-18)")
    safety_level: str = Field(default="safe", description="Safety level: safe, review, blocked")
    content_filtering_enabled: bool = Field(default=True, description="Enable content filtering")
    interaction_logging_enabled: bool = Field(default=True, description="Enable interaction logging")
    data_retention_days: int = Field(default=90, ge=1, le=2555, description="Data retention period in days")
    favorite_topics: List[str] = Field(default=[], description="Child's favorite topics")


class ChildUpdateRequest(BaseModel):
    name: Optional[str] = Field(None, min_length=1, max_length=100)
    safety_level: Optional[str] = Field(None, description="Safety level: safe, review, blocked")
...
async def get_children(
    ...
    try:
        ...
        try:
            ...
            raise ValidationError(f"Invalid parent ID format: {current_user.get('id', 'missing')}")
        ...
        stmt = select(Child).where(
            ...
            selectinload(Child.conversations).selectinload(Conversation.interactions),
            ...
            try:
                ...
                if hasattr(child, 'conversations') and child.conversations:
                    for conversation in child.conversations:
                        if hasattr(conversation, 'interactions') and conversation.interactions:
                            total_interactions += len(conversation.interactions)
                            latest_in_conv = max(conversation.interactions, key=lambda x: x.timestamp)
                            if not last_interaction or latest_in_conv.timestamp > last_interaction:
                ...
                safety_score = 100.0
                safety_score -= min(unresolved_safety_reports * 10.0, 50.0)  # Max 50 point penalty
...
async def get_child_interactions(
    ...
    try:
        ...
        interactions_stmt = select(Interaction).join(
            ...
            joinedload(Interaction.conversation)
        ).order_by(
        ...
        logger.info(f"Retrieved {len(interactions_db)} interactions for child {child_id}")
        ...
        for interaction in interactions_db:
            try:
                ...
                logger.error(f"Error processing interaction {interaction.id}: {str(interaction_error)}")
...
async def get_safety_alerts(
    ...
    try:
        ...
        stmt = select(SafetyReport, Child.name.label('child_name')).join(
            Child, SafetyReport.child_id == Child.id
        ).where(
            and_(
                Child.parent_id == parent_uuid,
                Child.is_deleted == False,
        ...
        logger.info(f"Retrieved {len(alert_rows)} safety alerts for parent {parent_uuid}")
        ...
        for safety_report, child_name in alert_rows:
            try:
                ...
                requires_immediate_attention = (
                    safety_report.severity in ['high', 'critical'] and 
                    not safety_report.resolved and
                    (datetime.utcnow() - safety_report.timestamp).hours < 24
                )
                ...
                logger.error(f"Error processing safety alert {safety_report.id}: {str(alert_error)}")
        ...
        logger.error(f"Unexpected error fetching safety alerts: {str(e)}", exc_info=True)
        await db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def resolve_safety_alert(
    ...
    try:
        ...
        if (safety_report.severity == 'critical' and 
            safety_report.alert_type in ['emergency', 'abuse_suspected']):
            logger.warning(
        ...
        if child and safety_report.severity in ['high', 'critical']:
            ...
            logger.info(f"Child {child.id} has {unresolved_count} remaining unresolved alerts")
        ...
        logger.error(f"Unexpected error resolving safety alert: {str(e)}", exc_info=True)
        await db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_dashboard_stats(
    ...
    try:
        ...
        active_children_stmt = select(
            func.count(func.distinct(Child.id))
        ).select_from(
            Child.join(Conversation).join(Interaction)
        ).where(
            and_(
                Child.parent_id == parent_uuid,
                Child.is_deleted == False,
        ...
        interactions_today_stmt = select(
            func.count(Interaction.id)
        ).select_from(
            Interaction.join(Conversation).join(Child)
        ).where(
            and_(
                Child.parent_id == parent_uuid,
                Child.is_deleted == False,
        ...
        unresolved_alerts_stmt = select(
            func.count(SafetyReport.id)
        ).select_from(
            SafetyReport.join(Child)
        ).where(
            and_(
                Child.parent_id == parent_uuid,
                Child.is_deleted == False,
                ...
                SafetyReport.is_deleted == False,
                SafetyReport.resolved == False,
                SafetyReport.severity.in_(['medium', 'high', 'critical'])  # Only significant alerts
            )
        )
        ...
        avg_safety_stmt = select(
            func.coalesce(
                func.avg(
                    case(
                        (Interaction.flagged == True, Interaction.safety_score * 0.8),  # Reduce flagged scores
            ...
            and_(
                ...
                Conversation.is_deleted == False,
                Interaction.is_deleted == False,
                Interaction.timestamp >= today_start - timedelta(days=7)  # Last 7 days for better average
            )
        )
...
async def create_safety_report(
    ...
    try:
        ...
        logger.error(f"Unexpected error creating safety report: {str(e)}", exc_info=True)
        await db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def create_child(
    ...
    try:
        ...
        if child_data.estimated_age < 13 and not child_data.interaction_logging_enabled:
            raise BusinessLogicError("Children under 13 must have interaction logging enabled (COPPA compliance)")
        ...
        new_child = Child(
            ...
            content_preferences={'language': 'age_appropriate', 'content_type': 'educational'},
            created_by=parent.id,
            metadata_json={
                'created_via': 'parent_dashboard',
...
async def update_child(
    ...
    try:
        ...
        if child_data.safety_level and child_data.safety_level not in ['safe', 'review', 'blocked']:
            raise ValidationError(f"Invalid safety level: {child_data.safety_level}")
        
        # COPPA compliance check for interaction logging
        if (child_data.interaction_logging_enabled is not None and 
            child.estimated_age < 13 and 
            not child_data.interaction_logging_enabled):
            raise BusinessLogicError("Children under 13 must have interaction logging enabled (COPPA compliance)")
        ...
        if child_data.safety_level is not None:
            changes['safety_level'] = {'old': child.safety_level.value, 'new': child_data.safety_level}
            child.safety_level = SafetyLevel(child_data.safety_level)
            
        if child_data.content_filtering_enabled is not None:
            changes['content_filtering_enabled'] = {'old': child.content_filtering_enabled, 'new': child_data.content_filtering_enabled}
        ...
        if child_data.interaction_logging_enabled is not None:
            changes['interaction_logging_enabled'] = {'old': child.interaction_logging_enabled, 'new': child_data.interaction_logging_enabled}
            child.interaction_logging_enabled = child_data.interaction_logging_enabled
            
        if child_data.data_retention_days is not None:
            changes['data_retention_days'] = {'old': child.data_retention_days, 'new': child_data.data_retention_days}
            child.data_retention_days = child_data.data_retention_days
            
        if child_data.favorite_topics is not None:
            changes['favorite_topics'] = {'old': child.favorite_topics, 'new': child_data.favorite_topics}
        ...
        child.metadata_json['update_history'] = child.metadata_json.get('update_history', [])
...
async def delete_child(
    ...
    try:
        ...
        if hard_delete:
            ...
            child.scheduled_deletion_at = datetime.utcnow() + timedelta(days=30)  # 30-day recovery window
            ...
            child.metadata_json['recovery_until'] = child.scheduled_deletion_at.isoformat()
        ...
        return {
            ...
            "recovery_until": child.scheduled_deletion_at.isoformat() if not hard_delete else None
        }
    ...
    try:
        ...
        child_stmt = select(Child).where(
            and_(
                ...
                Child.parent_id == parent.id,
                Child.is_deleted == True,
                Child.scheduled_deletion_at > datetime.utcnow()  # Within recovery window
            )
        )
