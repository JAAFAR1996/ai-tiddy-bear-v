...
# Create router
esp32_router = APIRouter(prefix="/esp32", tags=["ESP32"])
...
@esp32_router.get("/health")
async def esp32_health_check():
    """
    Health check endpoint for ESP32 Chat Server.
    
    Returns:
        JSON with server health status, active sessions, and service availability
    """
    try:
        health_status = await esp32_production_runner.health_check()
