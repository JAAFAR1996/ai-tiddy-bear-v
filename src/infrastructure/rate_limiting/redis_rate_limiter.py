...
class RedisRateLimiter:
    ...
    def _load_rate_limit_configs(self, default_config: Optional[Dict[str, Any]]) -> Dict[str, RateLimitConfig]:
    ...
    async def check_rate_limit(
        ...
        try:
            ...
            if adjusted_config.algorithm == RateLimitAlgorithm.SLIDING_WINDOW:
                result = await self._check_sliding_window(redis_key, adjusted_config, tokens_requested)
            elif adjusted_config.algorithm == RateLimitAlgorithm.TOKEN_BUCKET:
                result = await self._check_token_bucket(redis_key, adjusted_config, tokens_requested)
            elif adjusted_config.algorithm == RateLimitAlgorithm.FIXED_WINDOW:
                result = await self._check_fixed_window(redis_key, adjusted_config, tokens_requested)
            elif adjusted_config.algorithm == RateLimitAlgorithm.ADAPTIVE:
                result = await self._check_adaptive(redis_key, adjusted_config, tokens_requested, user_id)
            else:
            ...
            if not result.allowed:
                await self._record_violation(
                    user_id, user_type, limit_type, 
                    result.current_usage or 0, adjusted_config.max_requests,
    ...
    async def _check_sliding_window(
        ...
        return RateLimitResult(
            ...
            retry_after_seconds=int(reset_timestamp - current_time) if not allowed else None,
            limit_type=config.limit_type,
            current_usage=config.max_requests - remaining if allowed else config.max_requests
        )
    ...
    async def _check_adaptive(
        ...
        if violation_count > 5:
            ...
            adaptive_limit = min(adaptive_limit * 2, config.max_requests * 3)  # Double limit, cap at 3x
        ...
        result = await self._check_sliding_window(redis_key, temp_config, tokens_requested)
    ...
    def _adjust_config_for_age(self, config: RateLimitConfig, child_age: Optional[int]) -> RateLimitConfig:
    ...
    async def _detect_concerning_patterns(self, child_id: str, limit_type: LimitType) -> bool:
        ...
        timestamps = [float(activity.split(':')[0]) for activity in recent_activity[:20]]
    ...
    async def _check_abuse_patterns(self, user_id: str, user_type: UserType):
        ...
        violation_key = f"{self.key_prefix}:violations:{user_id}"
        recent_violations = await self.redis.lrange(violation_key, 0, 19)  # Last 20 violations
        ...
        if len(recent_violations) >= 10:
            ...
            if len(timestamps) >= 10:
                ...
                if time_span.total_seconds() < 3600:  # 10 violations in 1 hour
                    await self._apply_temporary_ban(user_id, user_type, duration_hours=24)
    
    async def _apply_temporary_ban(self, user_id: str, user_type: UserType, duration_hours: int):
    ...
    def _generate_redis_key(self, user_id: str, user_type: UserType, limit_type: LimitType) -> str:
        """Generate Redis key for rate limiting."""
        # Hash user_id for privacy
        user_hash = hashlib.sha256(user_id.encode()).hexdigest()[:16]
        return f"{self.key_prefix}:{user_type.value}:{limit_type.value}:{user_hash}"
    
    async def get_user_limits_status(self, user_id: str, user_type: UserType) -> Dict[LimitType, RateLimitResult]:
    ...
    async def get_violation_summary(self, hours: int = 24) -> Dict[str, Any]:
        ...
        for violation in recent_violations:
            ...
            summary['by_limit_type'][limit_type] = summary['by_limit_type'].get(limit_type, 0) + 1
        ...
        summary['top_violators'] = dict(
            sorted(summary['top_violators'].items(), key=lambda x: x[1], reverse=True)[:10]
        )
...
def create_redis_rate_limiter(
    ...
    if child_protection_mode:
        ...
        default_config = {
            ...
            'child:ai_requests_per_hour': {'max_requests': 30},  # Limited AI interaction
        }
