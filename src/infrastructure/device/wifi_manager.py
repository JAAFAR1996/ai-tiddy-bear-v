...
class ESP32WiFiManager:
    ...
    async def _set_state(self, new_state: WiFiState, reason: Optional[str] = None):
        ...
        with self._state_lock:
            if self._state != new_state:
                ...
                # Log state change
                logger.info(
                    f"WiFi state changed: {old_state.value} -> {new_state.value}"
    ...
    async def connect(
        ...
        try:
            ...
            if network_info and network_info.signal_strength < self.signal_threshold:
                logger.warning(
                    f"Weak signal for {ssid}: {network_info.signal_strength} dBm"
                )
            ...
            logger.info(
                f"Attempting to connect to {ssid} (attempt {self._connection_attempts})"
            )
            ...
            if success:
                ...
                await self._handle_connection_failure(
                    ssid, ConnectionFailureReason.UNKNOWN_ERROR
                )
                return False
            ...
            await self._handle_connection_failure(
                ssid, ConnectionFailureReason.UNKNOWN_ERROR, str(e)
            )
            return False
    ...
    async def _get_connection_info_linux(self) -> Optional[WiFiConnectionInfo]:
        ...
        network_info = (
            self._available_networks.get(self._target_network[0])
    ...
    async def _handle_connection_failure(
        self, ssid: str, reason: ConnectionFailureReason, details: Optional[str] = None
    ):
        ...
        # Log failure
        logger.error(
            f"Connection to {ssid} failed: {reason.value}"
        ...
        if (
            self.auto_reconnect
            and self._connection_attempts < self.max_reconnect_attempts
        ):
            logger.info(
                f"Scheduling reconnection attempt {self._connection_attempts + 1}"
            )
    ...
    def _save_network(
        self, ssid: str, password: Optional[str], security: Optional[WiFiSecurity]
    ):
    ...
    async def disconnect(self, forget_network: bool = False) -> bool:
        ...
        try:
            current_ssid = (
                self._current_connection.ssid if self._current_connection else "Unknown"
            )
        ...
        try:
            ...
            if strength is not None:
                ...
                self._signal_history = [
                    (time, signal)
                    for time, signal in self._signal_history
                    if time > cutoff
                ]
    ...
    async def stop_monitoring(self):
        ...
        tasks_to_cancel = [
            task
            for task in [self._monitor_task, self._scan_task, self._reconnect_task]
            ...
            try:
                if self.is_connected:
                    ...
                    if quality.get("signal_strength_dbm", 0) < self.signal_threshold:
                        logger.warning(
                            f"Weak signal detected: {quality['signal_strength_dbm']} dBm"
                        )

                await asyncio.wait_for(
                    self._shutdown_event.wait(), timeout=10  # Monitor every 10 seconds
                )
