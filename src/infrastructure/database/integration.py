...
from .repository import get_user_repository, get_child_repository, get_conversation_repository
...
class DatabaseMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app):
        super().__init__(app)
        self.enable_child_protection = config_manager.get_bool("CHILD_DATA_PROTECTION", True)
        self.log_all_queries = config_manager.get_bool("LOG_ALL_DATABASE_QUERIES", False)
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        try:
            ...
            if hasattr(request.state, 'database_queries') and request.state.database_queries:
                performance_logger.info(
                    ...
                    extra={
                        ...
                        "queries": request.state.database_queries if self.log_all_queries else []
                    }
                )
...
async def get_user_repo():
    ...
    return await get_user_repository()
...
def add_database_health_endpoints(app: FastAPI):
    ...
    async def database_health_summary():
        ...
        try:
            ...
            raise HTTPException(status_code=503, detail="Database health summary failed")
