...
class DeviceStatus(PyEnum):
    """Device status enumeration (must match DB enum values)."""
    # DB enum 'device_status' stores uppercase values.
    PENDING = "PENDING"
    ACTIVE = "ACTIVE"
    INACTIVE = "INACTIVE"
    ERROR = "ERROR"
...
class Device(BaseModel):
    ...
    # Use existing PostgreSQL enum type name 'device_status' (not the autogenerated 'devicestatus')
    status = Column(
        PGEnum(DeviceStatus, name="device_status", create_type=True),
        nullable=False,
