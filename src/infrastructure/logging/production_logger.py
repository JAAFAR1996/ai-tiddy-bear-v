...
class SecurityFilter(logging.Filter):
    ...
    SENSITIVE_KEYS = {
        ...
        'authorization', 'cookie', 'session', 'csrf', 'ssn',
            ...
            for sensitive in self.SENSITIVE_KEYS:
                if sensitive in msg_lower:
                    ...
                    record.msg = record.msg.replace(
                        record.msg[msg_lower.find(sensitive):msg_lower.find(sensitive) + len(sensitive) + 20],
                        f"{sensitive}=***MASKED***"
                    )
...
class COPPAComplianceFilter(logging.Filter):
    ...
    CHILD_DATA_KEYS = {
        'child_name', 'child_id', 'child_age', 'child_email',
        'parent_email', 'guardian_email', 'school', 'address',
        'phone', 'location', 'ip_address'
    }
