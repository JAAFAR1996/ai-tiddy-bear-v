...
from .logging_middleware import (
    RequestLoggingMiddleware, SecurityLoggingMiddleware,
    ChildSafetyLoggingMiddleware, setup_logging_middleware
)
...
class LoggingIntegration:
    ...
    async def start(self):
        """Start logging integration services."""
        try:
    ...
    async def stop(self):
        ...
        try:
            ...
            system_logger.info("Logging integration stopped", category=LogCategory.SYSTEM)
            
        except Exception as e:
            self.logger.error("Error stopping logging integration", error=e)
    
    async def _process_logs(self):
        """Background task to process and forward logs."""
        while True:
            try:
    ...
    def _update_metrics(self):
        ...
        # This would typically query log storage for metrics
        # For now, increment based on activity
...
def create_logging_app() -> FastAPI:
    ...
    app = FastAPI(
        title="AI Teddy Bear Logging API",
        version="1.0.0",
