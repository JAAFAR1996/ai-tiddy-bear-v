...
class SecurityStartupHooks:
    ...
    def run_environment_security_validation(self) -> bool:
        ...
        try:
            ...
            if not is_safe:
                self.logger.critical("💥 SECURITY VALIDATION FAILED - Application startup blocked")
                return False
        ...
        try:
            ...
            if environment == "production":
                ...
                if not private_key or not public_key:
                    self.logger.critical("💥 Production requires JWT_PRIVATE_KEY and JWT_PUBLIC_KEY")
                    return False
            elif not jwt_secret:
    ...
    def check_production_readiness(self) -> bool:
        ...
        try:
            ...
            if missing_vars:
                self.logger.critical(f"💥 Missing production environment variables: {missing_vars}")
                return False
    ...
    def run_all_security_checks(self) -> bool:
        ...
        for check_name, check_func in checks:
            ...
            if not check_func():
                self.logger.critical(f"💥 {check_name} validation FAILED - Startup blocked")
                return False
