...
class RouteMonitor:
    ...
    def scan_routes(self) -> Dict[str, any]:
        ...
        for route in self.app.routes:
            if isinstance(route, APIRoute):
                ...
                self._analyze_route_characteristics(
                    path, methods, route_name, tags,
                    routes_by_path, routes_by_prefix, auth_patterns,
                    health_routes, api_routes, web_routes
                )
        ...
        logger.info(f"✅ Route scan completed: {len(routes_by_path)} routes, {len(self.conflicts)} conflicts")
        return scan_results
    
    def _analyze_route_characteristics(
        self, path: str, methods: Set, route_name: str, tags: List,
        routes_by_path: Dict, routes_by_prefix: Dict, auth_patterns: Dict,
        health_routes: List, api_routes: List, web_routes: List
    ):
        ...
        if path in routes_by_path:
            ...
            if method_conflicts:
                self.conflicts.append(RouteConflict(
                    "method_conflict",
                    path,
                    {
    ...
    def _detect_prefix_overlaps(self, routes_by_prefix: Dict):
        ...
        api_prefixes = [prefix for prefix in routes_by_prefix.keys() if prefix.startswith("/api/")]
        ...
        if len(v1_routes) > 3:  # Threshold for too many v1 prefixes
            self.conflicts.append(RouteConflict(
                ...
                "/api/v1/*",
                {
                    ...
                    "route_count": sum(len(routes_by_prefix[prefix]) for prefix in v1_routes)
                }
            ))
    ...
    def validate_route_organization(self) -> Dict[str, any]:
        ...
        if critical_conflicts:
            ...
            validation_results["critical_issues"] = [c.to_dict() for c in critical_conflicts]
    ...
    def _generate_recommendations(self, validation_results: Dict, scan_results: Dict):
        ...
        if api_routes > 0:
            versioned_ratio = len([r for r in scan_results["routes_by_path"].keys() 
                                 if "/v1/" in r]) / api_routes
            if versioned_ratio < 0.8:
                recommendations.append({
                    "priority": "HIGH",
                    "category": "API_VERSIONING",
                    "message": "Add version prefixes to all API routes (e.g., /api/v1/)",
                    "affected_routes": api_routes - int(api_routes * versioned_ratio)
                })
    ...
    def generate_route_report(self) -> str:
        ...
        for prefix, routes in scan_results["routes_by_prefix"].items():
            ...
            for route in routes[:5]:  # Show first 5 routes
                ...
                report_lines.append(f"- `{route['path']}` - [{methods_str}] - {route['name']}")
    ...
    def get_route_summary(self) -> Dict[str, any]:
        ...
        return {
            ...
            "status": "HEALTHY" if scan_results["conflicts_detected"] == 0 else "ISSUES_DETECTED"
        }
...
def validate_application_routes(app: FastAPI) -> bool:
    ...
    if status == "HEALTHY":
        ...
        logger.warning(f"⚠️ Route validation PASSED with minor issues: {total_routes} routes, {conflicts} minor conflicts")
        return True
    elif status == "WARNING":
        logger.warning(f"⚠️ Route validation WARNING: {total_routes} routes, {conflicts} conflicts")
        for warning in validation_results["warnings"]:
            logger.warning(f"   - {warning['type']}: {warning['path']}")
        return False
    else:  # CRITICAL
        logger.error(f"❌ Route validation FAILED: {total_routes} routes, {conflicts} critical conflicts")
        for issue in validation_results["critical_issues"]:
            logger.error(f"   - {issue['type']}: {issue['path']}")
        return False


def generate_route_documentation(app: FastAPI, output_file: Optional[str] = None) -> str:
    """
