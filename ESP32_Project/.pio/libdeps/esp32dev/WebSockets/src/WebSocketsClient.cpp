        ...
        if(ok) {
            switch(client->cCode) {
                ...
                case 200:
                    ...
                    // falls through
                case 403:    ///< Forbidden
                    DEBUG_WEBSOCKETS("[WS-Client][handleHeader] 403 Forbidden - attempting re-authentication/login\n");
                    handleLogin(this);
                    // بعد محاولة المصادقة، أعد محاولة الاتصال
                    clientDisconnect(client);
                    // إعادة المحاولة ستحدث تلقائيًا في الحلقة الرئيسية
                    break;
...
/**
 * دالة إنتاجية لإعادة المصادقة/تسجيل الدخول عند رفض الاتصال
 */
void WebSocketsClient::handleLogin(WebSocketsClient* client) {
    // إرسال توكن أو بيانات دخول جديدة
    String authToken = getAuthToken();
    if (authToken.length() > 0) {
        DEBUG_WEBSOCKETS("[WS-Client][handleLogin] Sending new auth token to server\n");
        client->setAuthorization(authToken.c_str());
    } else {
        DEBUG_WEBSOCKETS("[WS-Client][handleLogin] No auth token available, cannot re-authenticate\n");
    }
}

/**
 * دالة للحصول على التوكن من النظام - إنتاجية
 */
String WebSocketsClient::getAuthToken() {
    // محاولة جلب التوكن من ملف إعدادات داخلي أولاً
    String token = "";
#if defined(ESP32)
    if (SPIFFS.begin(true)) {
        ...
        if (file) {
            ...
            token.trim();
            file.close();
            DEBUG_WEBSOCKETS("[WS-Client][getAuthToken] Token loaded from SPIFFS file\n");
        }
        SPIFFS.end();
    }
#endif

    // إذا لم يوجد توكن في الملف، محاولة جلبه من Preferences (نظام المصادقة الموجود)
    if (token.length() == 0) {
        // استخدام نفس نظام Preferences المستخدم في security.cpp
        Preferences prefs;
        ...
        if (token.length() > 0) {
            DEBUG_WEBSOCKETS("[WS-Client][getAuthToken] Token loaded from security preferences\n");
        }
    }
    
    // إذا لم يوجد في أي مكان، استخدم القيمة الافتراضية
    if (token.length() == 0) {
        token = _client.base64Authorization;
        DEBUG_WEBSOCKETS("[WS-Client][getAuthToken] Using default base64 authorization\n");
    }
